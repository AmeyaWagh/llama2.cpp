...
TEST(testTensor, creation) {
    ...
    Tensor<CPU, int> tensor(shape, data);
    for (auto i = 0; i < 2; ++i) {
        for (auto j = 0; j < 3; ++j) {
    ...
    Shape other_shape = {2, 3};
    EXPECT_EQ(shape, other_shape);
}

TEST(testTensor, matmul) {
    std::vector<float> w_data = {1, 2, 3, 4, 5, 6};
    Tensor<CPU, float> w(Shape(2UL, 3UL), w_data);

    std::vector<float> x_data = {7, 8, 9};
    Tensor<CPU, float> x(Shape(3UL), x_data);

    Tensor<CPU, float> out(Shape(2UL));
...
TEST(testTensor, slice) {
    ...
    w_data0[0] = 1;
    w_data0[1] = 2;
    Tensor<CPU, float> tensor0(shape0, w_data0);
    ...
    Shape shape(2UL, 3UL, 5UL);  // CHW
    std::vector<float> w_data(shape.size());
    for (auto i = 0; i < shape.size(); ++i) {
        w_data[i] = i;
    }
    Tensor<CPU, float> tensor(shape, w_data);
    ...
    //                                           C  -> {H,W}
    TensorView<float> s1 = tensor.slice(0);  // TensorView(ptr+0*(15), Shape(3,5)) stride {5,1}
    TensorView<float> s2 = tensor.slice(1);  // TensorView(ptr+1*(15), Shape(3,5)) stride {5,1}

    //                                           C, H  -> {W}
    TensorView<float> s3 = tensor.slice(1, 0);  // TensorView(ptr+1*(15)+0*(5), Shape(5)) stride {1}
    TensorView<float> s4 = tensor.slice(1, 1);  // TensorView(ptr+1*(15)+1*(5), Shape(5)) stride {1}

    //                                           C, H, W  -> {}
    TensorView<float> s5 = tensor.slice(1, 1, 2);  // TensorView(ptr+1*(15)+1*(5)+2*(1), Shape(1)) stride {}
